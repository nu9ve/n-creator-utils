...
from novation_launchpad.charset import *
		...
		if self.devIn is None:
			try:
				...
				if bufferSize is None:
					...
					# for experiments...
					self.devIn = rtmidi.MidiIn( queue_size_limit=bufferSize )
					self.devIn.open_port( midi_id )
			except:
				self.devIn = None
				return False
...
#			for n in range( (len(string) + 1) * 8 - 1, 0, -1 ):
	...
	def ButtonStateRaw( self ):
		if self.msg:
			return [ self.msg[1] if self.msg[0] == 144 else self.msg[1] + 96, True if self.msg[2] > 0 else False ]
		else:
			return []
	...
	def ButtonStateXY( self ):
		if self.msg:
			...
			if self.msg[0] == 144:
				...
				return [ self.msg[1] - 104, 0, True if self.msg[2] > 0 else False ]
...
class LaunchControlXL( LaunchpadBase ):
	...
	def InputStateRaw( self ):
		if self.msg:
			...
			#--- pressed
			if    self.msg[0] == 144:
				return [ self.msg[1], True, 127 ]
			#--- released
			elif  self.msg[0] == 128:
				return [ self.msg[1], False, 0 ]
			#--- potentiometers and the four cursor buttons
			elif  self.msg[0] == 176:
				# --- cursor buttons
				if self.msg[1] >= 104 and self.msg[1] <= 107:
					if self.msg[2] > 0:
						return [ self.msg[1], True, self.msg[2] ]
					else:
						return [ self.msg[1], False, 0 ]
				# --- potentiometers
				else:
					return [ self.msg[1], self.msg[2], 0 ]
			else:
				return []
		else:
			return []
	...
	def InputStateRaw( self ):
		if self.msg:
			...
			#--- potentiometers and the four cursor buttons
				...
				if self.msg[1] >= 104 and self.msg[1] <= 109:
					if self.msg[2] > 0:
						return [ self.msg[1], True, 127 ]
					else:
						return [ self.msg[1], False, 0 ]
...
class Dicer( LaunchpadBase ):
	...
	def ButtonStateRaw( self ):
		if self.msg:
			...
			#--- button on master
			if   self.msg[0] >= 154 and self.msg[0] <= 156:
				butNum = self.msg[1]
				if butNum >= 60 and butNum <= 69:
					butNum -= 59
					butNum += 10 * ( self.msg[0]-154 )
					if self.msg[2] == 127:
					...
					return []
			#--- button on master
			elif self.msg[0] >= 157 and self.msg[0] <= 159:
				butNum = self.msg[1]
				if butNum >= 60 and butNum <= 69:
					butNum -= 59
					butNum += 100 + 10 * ( self.msg[0]-157 )
					if self.msg[2] == 127:
						return [ butNum, True, 127 ]
					else:
						return [ butNum, False, 0  ]
